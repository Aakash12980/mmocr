...
from mmocr.core.visualize import det_recog_show_result, calculate_angle
...
class MMOCR:
    ...
    def readtext(self,
        ...
        if self.detect_model and self.recog_model:
            ...
            for res in det_recog_result:
                ang_list = []
                for box in (res['result']):
                    angle = calculate_angle(box['box'])
                    ang_list.append(angle)
                print(f"\n\nAngles for each bounding box:\n {sorted(ang_list)}\n\n")
                print(f"Maximum value: {max(ang_list)} | Minimim value: {min(ang_list)}")
                print(f"Most frequent Angle: {max(set(ang_list), key = ang_list.count)}")
                print(f"Total bounding boxes found: {len(ang_list)}")
                print(f"Mean value of all angles: {np.array(ang_list).mean()}\n\n")
        ...
        for arr, output, export, det_recog_result in zip(
            ...
            if output or args.imshow:
                ...
                if args.imshow and not self.kie_model:
                    ...
                    mmcv.imwrite(res_img, './output/out.jpg')
            if not args.details:
                simple_res = {}
...
def main():
    args = parse_args()
    ocr = MMOCR(**vars(args))
    results = ocr.readtext(**vars(args))
